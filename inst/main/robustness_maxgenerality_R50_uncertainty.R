...
dd_mg <- readRDS("results/maxgenerality_uncertainty.RDS")

fw_list <- unique(dd_mg$fw_name)
...
for(foodweb in fw_list){
  ...
  dd_mg_real <- dd_mg_real %>%
    mutate(total_ext = (n_ext + acc_sec_ext)/S)
  
  robustness <- NA
  N_row <- dim(dd_mg_real)[1]
  for(i in 1:N_row){
    ith_row <- dd_mg_real[i,]
  ...
  for(nsim_local in 1:1000){
    ...
    dd_mg_ADBM_ABC <- dd_mg %>%
      filter(fw_name == foodweb, type == "ADBM", nsim == nsim_local) %>%
      mutate(total_ext = (n_ext + acc_sec_ext)/S)
    
    robustness <- NA
    N_row <- dim(dd_mg_ADBM_ABC)[1]
    for(i in 1:N_row){
      ith_row <- dd_mg_ADBM_ABC[i,]
      if(ith_row$total_ext >= 0.5){
        robustness <- ith_row$n_ext/ith_row$S
        break
      }
...
# saveRDS(object = rob_all, file = "results/robustness_R50_maxgenerality_uncertainty.RDS")
