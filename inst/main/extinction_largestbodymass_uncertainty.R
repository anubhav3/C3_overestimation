# 2022.02.23
# We simulated primary extinction by removing largest body mass species in all the 1000 food web predicted by the ADBM 
...
for(i in fw_ind){
  ...
  real_ext_mc <- largestbodymass_ext(net = real_pred_mat, bodysize = bodysize_real)
  ...
  for(j in 1:1000){
    ...
    ADBM_ABC_ext_mc <- largestbodymass_ext(net = ADBM_ABC_pred_mat, bodysize = bodysize_ADBM)
...
# saveRDS(object = dd_la, file = "results/largestbodymass_uncertainty.RDS")
