# 2022.07.25
# We simulated primary extinction by removing random species (without basal) in all the 1000 food web predicted by the ADBM 
...
for(i in fw_ind){
  ...
  real_ext_rand <- random_ext_parallel_wo_basal(net = real_pred_mat, nsim = Nsim, n_cores = 5)
  ...
  for(j in 1:1000){
    ...
    ADBM_ABC_ext_rand <- random_ext_parallel_wo_basal(net = ADBM_ABC_pred_mat, nsim = Nsim, n_cores = 5)
