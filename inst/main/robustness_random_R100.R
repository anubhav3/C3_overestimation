# 2022.04.28
# We compute robustness R100 (the proportion of species that have to be removed to achieve a total loss of all species)
...
dd_rand_raw <- readRDS("results/random_raw_uncertainty.RDS")
...
for(i in 1:N){
  ...
  for(i in 1:N_row){
    ith_row <- dd_fil[i,]
    if(ith_row$acc_tot_ext >= 1.0){
      robustness <- ith_row$acc_pri_ext/ith_row$S
      break
    }
  }
  
  pred_mat <- readRDS(paste0("data/", type_fw, "_ABC_predicted_foodwebs/", foodweb, ".pred_mat.RDS"))
  S <- dim(pred_mat)[1]
...
# saveRDS(object = rob_all, file = "results/robustness_R100_random.RDS")

# rob_all <- readRDS("results/robustness_R100_random.RDS")
...
# saveRDS(object = plot_rob_all, file = "results/ggplot_rob_R100_random.RDS")


# ggsave(plot = plot_rob_all, filename = "results/plot_robustness_R100_random.png", width = 11, height = 6)
