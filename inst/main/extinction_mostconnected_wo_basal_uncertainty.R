# 2022.07.22
# We simulated primary extinction by removing most connected species in all the 1000 food web predicted by the ADBM 
...
for(i in fw_ind){
  ...
  real_ext_mc <- most_connected_ext_wo_basal(net = real_pred_mat)
  ...
  for(j in 1:1000){
    ...
    ADBM_ABC_ext_mc <- most_connected_ext_wo_basal(net = ADBM_ABC_pred_mat)
...
# saveRDS(object = dd_mc, file = "results/most_connnected_wo_basal_uncertainty_new.RDS")
