...
# Compute robustness (R50) of ADBM predicted food webs for most connected scenario (without basal)
...
dd_lc <- readRDS("results/least_connnected_wo_basal_uncertainty.RDS")
...
for(foodweb in fw_list){
  ...
  S_wo_basal <- dd_lc_emp$S_wo_basal[1]
  ...
  for(nsim_local in 1:1000){
    ...
    S_wo_basal <- dd_lc_ADBM_ABC$S_wo_basal[1]
...
# saveRDS(object = rob_all, file = "results/robustness_R50_leastconnected_wo_basal_uncertainty.RDS")
