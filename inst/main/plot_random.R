...
dd_rand_mod <- dd_rand_raw %>%
  ...
  group_by(fw_name, acc_pri_ext, type, S) %>%
  summarise(mean_acc_sec_ext = mean(acc_sec_ext), l_acc_sec_ext = min(acc_sec_ext), u_acc_sec_ext = max(acc_sec_ext)) %>%
  mutate(acc_pri_ext_by_S = acc_pri_ext/S, acc_sec_ext_by_S = mean_acc_sec_ext/S, l_acc_sec_ext_by_S = l_acc_sec_ext/S,
...
# ggsave(plot = plot_dd_random_mod, filename = "results/plot_random.png", width = 13, height = 6)
