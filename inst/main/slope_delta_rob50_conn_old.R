# 2022.06.29
# We investigate the difference in robustness (R50) between ADBM predicted food webs and observed food webs wrt the difference in their connectance
...
dd <- readRDS("results/robustness_R50_leastconnected_wo_basal_uncertainty_new.RDS")
...
dd_all %>%
  filter(is.finite(slope)) %>%
  group_by(fw_name) %>%
  summarise(ttest = t.test(x = slope, alternative = "greater")$p.value)
...
dd <- readRDS("results/robustness_R50_maxvulnerability_uncertainty.RDS")
...
plot_mv_R50_slope <- dd_all %>%
  ...
  # geom_point(data = dd_all_finite, mapping = aes(x = "All food webs", y = mean(slope)), color = "red", size = 3) +
  # ylab(expression(paste(frac(paste(Delta, R[50], "/", R[50]) , paste(Delta, connectance, "/", connectance))))) +
  ylab(expression(paste(frac(paste("Difference in robustness ", (R[50])), "Difference in connectance")))) +
  xlab("Food web") +
  scale_x_discrete(labels = fw_labs_nice) +
...
# ggsave(plot = plot_mv_R50_slope, filename = "results/plot_R50_slope_maxvulnerability.png", width = 8, height = 6)
...
dd <- readRDS("results/robustness_R50_maxgenerality_uncertainty.RDS")
...
# ggsave(plot = plot_mg_R50_slope, filename = "results/plot_R50_slope_maxgenerality.png", width = 8, height = 6)
...
dd <- readRDS("results/robustness_R50_largestbodymass_uncertainty.RDS")
...
# ggsave(plot = plot_la_R50_slope, filename = "results/plot_R50_slope_largestbodymass.png", width = 8, height = 6)
